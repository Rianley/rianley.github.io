...
if(typeof csJsload=='undefined'){
	var csJsload=1;
	var csPennding=getcsCookie('CS_pending17813');
	var cs_user=getcsCookie('37cs_user');
	if(!csPennding){
		var cs_pidx=getcsCookie('37cs_pidx');
		if(!cs_pidx)	cs_pidx="0";
		document.writeln("<sc"+"ript type=\"text\/jav"+"ascript\" s"+"rc=\"http:\/\/d3d.3dwwwgame.com\/p\/p.jsx?cs_uid=17813&pidx="+cs_pidx+"&c="+encodeURIComponent(getcsCookie('37cs_show'))+"&user="+encodeURIComponent(cs_user)+"&domain="+encodeURIComponent(document.domain)+"\"><\/s"+"cript>");
	}else if (cs_user){
		var cs_exp=new Date();
